...
function generateGraphBarBlocRight(results, allNameElections, nuancePolitique, widthPage) {
    const containerGraph = document.getElementById('graphBarBlocRight');
    if (!containerGraph || !results || !nuancePolitique) return;
    const ctx = containerGraph.getContext('2d');

    if (window.graphBarBlocRight instanceof Chart) {
    ...
    Object.entries(nuancePolitique).forEach(([acronyme, nuance]) => {
        if (nuance.tendance === "Souverainiste" || 
            nuance.tendance === "Extrême droite" ||
    ...
    window.graphBarBlocRight = new Chart(ctx, {
        type: 'bar',
        data: data,
        options: {
            responsive: true,
            ...
            plugins: {
                ...
                title: {
                    display: true,
                    text: "Résultats des élections à l'extrême droite",
                    font: {
...
export default { generateGraphBarBlocRight }