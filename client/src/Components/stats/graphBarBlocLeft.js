...
function generateGraphBarBlocLeft(results, allNameElections, nuancePolitique, widthPage) {
    const containerGraph = document.getElementById('graphBarBlocLeft');
    if (!containerGraph || !results || !nuancePolitique) return;
    const ctx = containerGraph.getContext('2d');

    if (window.graphBarBlocLeft instanceof Chart) {
    ...
    Object.entries(nuancePolitique).forEach(([acronyme, nuance]) => {
        if (nuance.tendance === "Gauche" || nuance.tendance === "Extrême gauche") {
            partiMap[acronyme] = {
                label: acronyme,
    ...
    window.graphBarBlocLeft = new Chart(ctx, {
        type: 'bar',
        data: data,
        options: {
            responsive: true,
            ...
            plugins: {
                ...
                title: {
                    display: true,
                    text: 'Résultats des élections à gauche',
                    font: {
...
export default { generateGraphBarBlocLeft }